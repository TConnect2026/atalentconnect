      ...
      <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-8 max-w-7xl">
        ...
        <Card style={{ border: '2px solid #1e3a5f' }}>
          ...
          <CardContent className="pt-6 space-y-6">
            {/* Search Details */}
            <div>
              <h3 className="text-base font-bold text-gray-900 mb-3 border-b pb-2">Search Details</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-1">Company Name</p>
                  <p className="text-sm text-gray-900">{search.company_name}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-1">Position Title</p>
                  <p className="text-sm text-gray-900">{search.position_title}</p>
                </div>
                {search.position_location && (
                  <div>
                    <p className="text-xs font-semibold text-gray-500 mb-1">Location</p>
                    <p className="text-sm text-gray-900">{search.position_location}</p>
                  </div>
                )}
                {search.reports_to && (
                  <div>
                    <p className="text-xs font-semibold text-gray-500 mb-1">Reports To</p>
                    <p className="text-sm text-gray-900">{search.reports_to}</p>
                  </div>
                )}
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-1">Open to Relocation</p>
                  <p className="text-sm text-gray-900">{search.open_to_relocation ? 'Yes' : 'No'}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-gray-500 mb-1">Relocation Package Available</p>
                  <p className="text-sm text-gray-900">{search.relocation_package_available ? 'Yes' : 'No'}</p>
                </div>
                {search.compensation_range && (
                  <div className="col-span-2">
                    <p className="text-xs font-semibold text-gray-500 mb-1">Compensation Range</p>
                    <p className="text-sm text-gray-900 whitespace-pre-wrap">{search.compensation_range}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {contacts.length > 0 && (
              <div>
                <h3 className="text-base font-bold text-gray-900 mb-3 border-b pb-2">Client Contacts</h3>
            ...
            {/* Pipeline Stages */}
            <div>
              <h3 className="text-base font-bold text-gray-900 mb-3 border-b pb-2">Pipeline Stages</h3>
            ...
            {documents.length > 0 && (
              <div>
                <h3 className="text-base font-bold text-gray-900 mb-3 border-b pb-2">Documents</h3>
                <div className="space-y-2">
                  {documents.map((doc) => (
                    <div
                      key={doc.id}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">{getDocumentTypeLabel(doc.type)}</p>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
