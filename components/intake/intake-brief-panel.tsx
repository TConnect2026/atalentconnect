...
interface IntakeBriefPanelProps {
  searchId: string
  onClose?: () => void
}

export function IntakeBriefPanel({ searchId, onClose }: IntakeBriefPanelProps) {
    ...
    (updatedBrief: IntakeBriefRecord) => {
      ...
      saveTimer.current = setTimeout(async () => {
        ...
        try {
          ...
          // Phase 1 mirror: keep the brief and Essentials in sync by writing
          // the shared fields onto the canonical `searches` row. Compensation
          // doesn't have a direct counterpart in the brief's snapshot today,
          // so it's left for full reconciliation later.
          const rb = updatedBrief.snapshot?.roleBasics
          if (rb) {
            const nullIfEmpty = (v: string | undefined | null) =>
              v && String(v).trim() ? v : null
            const searchesPatch: Record<string, any> = {
              position_title: nullIfEmpty(rb.officialTitle),
              reports_to: nullIfEmpty(rb.reportsTo),
              position_location: nullIfEmpty(rb.roleLocation),
              work_arrangement: nullIfEmpty(rb.workArrangement),
              launch_date: nullIfEmpty(rb.launchDate),
              target_fill_date: nullIfEmpty(rb.targetCloseDate),
              updated_at: new Date().toISOString(),
            }
            const { error: searchesError } = await supabase
              ...
              .eq('id', searchId)
            if (searchesError) {
              console.error('Searches mirror failed:', searchesError)
              throw searchesError
            }
          }
  ...
  if (step === 'start') {
    return (
      <div className="min-h-screen bg-[#FAF9F7]">
        {/* Header omitted in slide-over host */}
